...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    fullName: '', email: '', password: '', role: 'WORKER', phone: '', location: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Network / timeout — backend cold-starting on Render free tier
      if (!axiosErr.response || axiosErr.code === 'ECONNABORTED' || axiosErr.code === 'ERR_NETWORK') {
        setError('The server is waking up — this can take up to 60 seconds. Please try again in a moment.');
      } else if (axiosErr.response.status === 409) {
        setError('An account with this email already exists. Please sign in instead.');
      } else if (axiosErr.response.status === 400) {
        setError(axiosErr.response?.data?.message ?? 'Please check your details and try again.');
      } else if (axiosErr.response.status === 404) {
        setError('API endpoint not found. The backend may need a redeploy — check Render dashboard.');
      } else {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {ROLES.map(r => (
                  ...
                  >
                    <p className={`text-sm font-semibold ${form.role === r.value ? 'text-brand-600' : 'text-slate-700'}`}>
                      {r.label}
                    </p>
