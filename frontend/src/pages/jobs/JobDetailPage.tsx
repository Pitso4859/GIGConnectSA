...
export default function JobDetailPage() {
  ...
  const act = async (fn: () => Promise<unknown>, msg = '') => {
    setActionLoading(true); setError('');
    try { const r = await fn() as { data: { data: Job } }; setJob(r.data.data); if (msg) alert(msg); }
    catch (e: unknown) { setError((e as { response?: { data?: { message?: string } } }).response?.data?.message ?? 'Action failed'); }
  ...
  const submitRating = async () => {
    try {
      ...
      setRated(true);
    } catch (e: unknown) { setError((e as { response?: { data?: { message?: string } } }).response?.data?.message ?? 'Rating failed'); }
  };
